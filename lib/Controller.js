...
class Controller extends Core.cls( "Core.app.Application" ) {
	...
	_prepDependencies() {
		...
		ioc.singleton( "localFs", function( ixc ) {

			let localPathManager = ixc.resolve( "localPathManager" );
			let localFsAdapter   = ixc.resolve( "localFsAdapter" );

			return me.$spawn( "Core.fs.FileSystem", {
				pathManager       : localPathManager,
				fileSystemAdapter : localFsAdapter
			} );

		} );

		// Init the local file system adapter
		ioc.singleton( "localFsAdapter", function() {
	...
	async _loadPlugins() {
		...
		let mochaPluginPath = "/home/ec2-user/environment/core/core-cli-plugin-mocha"; // todo: use a path manager & path resolver
